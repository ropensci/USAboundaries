...
state_plane <- function(state, plane_id = NULL, type = c("epsg", "proj4")) {
  ...
  stopifnot(is.null(plane_id) || is.character(plane_id))
  if (is.null(plane_id) && type == "epsg") {
    item <- state_proj[state_proj$state == state & state_proj$statewide_proj == TRUE, "epsg"]
      ...
      item <- state_proj[state_proj$state == state & toupper(state_proj$zone) == toupper(plane_id), "epsg"]
    } else if (type == "proj4") {
      item <- state_proj[state_proj$state == state & toupper(state_proj$zone) == toupper(plane_id), "proj4_string"]
    }
